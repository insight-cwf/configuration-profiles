...
#
#   This script is designed to suppress the Microsoft AutoUpdate Required Data
#   Notice screen
#
...
for userName in ${allLocalUsers}; do
    # Identify the home folder location of all users with a UID greater than
    # 500.
	userHome=$(/usr/bin/dscl . -read \
        "/Users/$userName" NFSHomeDirectory 2>/dev/null | \
        /usr/bin/sed 's/^[^\/]*//g')

    # Verify that home folder actually exists.
    if [[ -d  "$userHome" ]]; then
        # If the home folder exists, sets the com.microsoft.autoupdate2.plist
        # file with the needed key and value.
     	/usr/bin/defaults write \
            "${userHome}/Library/Preferences/com.microsoft.autoupdate2.plist" \
            AcknowledgedDataCollectionPolicy RequiredDataOnly

        # This script is designed to be run with root privileges, so the
        # ownership of the com.microsoft.autoupdate2.plist file
        # and the enclosing directories are re-set to that of the account which
        # owns the home folder.
     	/usr/sbin/chown "$userName" "${userHome}/Library/"
     	/usr/sbin/chown "$userName" "${userHome}/Library/Preferences"
     	/usr/sbin/chown "$userName" \
            "${userHome}/Library/Preferences/com.microsoft.autoupdate2.plist"
    fi
done
